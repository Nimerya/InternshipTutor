...
public interface StudentInternshipRepository extends JpaRepository<StudentInternship, Long> {
    ...
    //List of all internships that are in progress (with respect to the student)
    @Query(value =  "SELECT si.student_id," +
            ...
            "        WHERE si.student_id = ?1 and si.completed = FALSE and si.accepted = TRUE ", nativeQuery = true)
    ...
    @Query(value =  "SELECT si.student_id," +
            ...
            "               si.id as id" +
            "        FROM student_internship si" +
            "        WHERE si.student_id = ?1 and si.completed = FALSE and si.accepted = FALSE ", nativeQuery = true)
